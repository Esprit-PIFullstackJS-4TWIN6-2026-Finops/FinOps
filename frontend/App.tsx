...
const A11Y_FONT_SCALE_STORAGE_KEY = 'finops_a11y_font_scale';
const A11Y_LARGE_UI_STORAGE_KEY = 'finops_a11y_large_ui';
const A11Y_SPEECH_STORAGE_KEY = 'finops_a11y_speech_enabled';
const A11Y_SPEECH_VOICE_STORAGE_KEY = 'finops_a11y_speech_voice_uri';
...
type AccessibilityCopy = {
  ...
  speechVoices: (count: number) => string;
  speechNoVoices: string;
  fontStatus: (scale: number) => string;
  largeUiOn: string;
  largeUiOff: string;
  speechOn: string;
  speechOff: string;
  speechVoice?: (name: string) => string;
  speechVoicePicker?: string;
  speechBrowserDefault?: string;
  speechPreparing?: (name: string) => string;
  speechSpeaking?: string;
  speechDone?: string;
  speechRetrying?: string;
  speechError?: (message: string) => string;
};

function getStoredFontScale(): number {
  const stored = Number(localStorage.getItem(A11Y_FONT_SCALE_STORAGE_KEY));
  return [0.9, 1, 1.1, 1.2].includes(stored) ? stored : 1;
}
...
function getSpeechTestSentence(lang: UiLang): string {
  if (lang === 'fr') {
    return 'Bonjour. Ceci est un test de lecture vocale. Si vous m entendez, le son fonctionne correctement.';
  }
  if (lang === 'ar') {
    return 'Speech test. If you can hear this message, the sound is working correctly.';
  }
  return 'Hello. This is a speech test. If you can hear this message, the sound is working correctly.';
}
...
function readAriaLabelledBy(element: HTMLElement): string {
  const labelledBy = normalizeTooltipText(element.getAttribute('aria-labelledby'));
...
function extractInteractiveTooltipText(element: HTMLElement): string {
  const explicit =
    normalizeTooltipText(element.getAttribute('data-tooltip')) ||
    normalizeTooltipText(element.getAttribute('aria-label')) ||
    normalizeTooltipText(element.getAttribute('title')) ||
    readAriaLabelledBy(element);
  if (explicit) return explicit;

  if (element instanceof HTMLInputElement || element instanceof HTMLSelectElement || element instanceof HTMLTextAreaElement) {
    const wrappedLabel = normalizeTooltipText(element.closest('label')?.textContent);
    ...
    return (
      normalizeTooltipText(element.placeholder) ||
      normalizeTooltipText(element.name) ||
      normalizeTooltipText(element.getAttribute('aria-describedby'))
    );
  }

  return normalizeTooltipText(element.innerText || element.textContent);
}

function getInteractiveTooltipTarget(target: EventTarget | null): HTMLElement | null {
  if (!(target instanceof HTMLElement)) return null;
  if (target.closest('[data-a11y-skip-tooltip="true"]')) return null;
  const interactive = target.closest<HTMLElement>('button, a, input, select, textarea, [role="button"], [role="option"], [role="tab"]');
  if (!interactive) return null;
  if (interactive.dataset.a11ySkipTooltip === 'true') return null;
  return interactive;
}

function getAccessibilityCopy(lang: UiLang): AccessibilityCopy {
  if (lang === 'fr') {
    return {
      panelTitle: 'Accessibilite',
      openPanel: 'Ouvrir les options d accessibilite',
      closePanel: 'Fermer les options d accessibilite',
      fontSize: 'Taille du texte',
      smaller: 'A-',
      normal: 'A',
      larger: 'A+',
      largerUi: 'Logo et avatars agrandis',
      speech: 'Lecture vocale au survol',
      speechTest: 'Tester la voix',
      speechUnsupported: 'Lecture vocale non prise en charge sur ce navigateur.',
      speechVoices: (count: number) => `Voix detectees: ${count}`,
      speechNoVoices: 'Aucune voix du navigateur detectee. Installe une voix Windows ou attends quelques secondes puis reteste.',
      speechVoice: (name: string) => `Voix active: ${name}`,
      speechVoicePicker: 'Choisir une voix',
      speechBrowserDefault: 'Voix du navigateur',
      speechPreparing: (name: string) => `Preparation de la voix ${name}`,
      speechSpeaking: 'Lecture vocale en cours',
      speechDone: 'Lecture vocale terminee',
      speechRetrying: 'La voix choisie ne demarre pas. Nouvelle tentative.',
      speechError: (message: string) => `Lecture vocale indisponible: ${message}`,
      fontStatus: (scale: number) => `Taille du texte ${Math.round(scale * 100)} pour cent`,
      largeUiOn: 'Mode visuel agrandi active',
      largeUiOff: 'Mode visuel agrandi desactive',
      speechOn: 'Lecture vocale activee',
      speechOff: 'Lecture vocale desactivee',
    };
  }

  if (lang === 'ar') {
    return {
      panelTitle: 'امكانية الوصول',
      openPanel: 'فتح خيارات امكانية الوصول',
      closePanel: 'اغلاق خيارات امكانية الوصول',
      fontSize: 'حجم النص',
      smaller: 'A-',
      normal: 'A',
      larger: 'A+',
      largerUi: 'تكبير الشعار والصور الرمزية',
      speech: 'قراءة صوتية عند المرور',
      speechTest: 'اختبار الصوت',
      speechUnsupported: 'القراءة الصوتية غير مدعومة في هذا المتصفح.',
      speechVoices: (count: number) => `الاصوات المكتشفة: ${count}`,
      speechNoVoices: 'لم يتم العثور على اصوات في المتصفح. ثبت صوتا في ويندوز او انتظر قليلا ثم اعد التجربة.',
      fontStatus: (scale: number) => `حجم النص ${Math.round(scale * 100)} بالمئة`,
      largeUiOn: 'تم تفعيل العرض المكبر',
      largeUiOff: 'تم تعطيل العرض المكبر',
      speechOn: 'تم تفعيل القراءة الصوتية',
      speechOff: 'تم تعطيل القراءة الصوتية',
    };
  }
  ...
  return {
    ...
    normal: 'A',
    larger: 'A+',
    largerUi: 'Larger logo and avatars',
    speech: 'Speak labels on hover',
    speechTest: 'Test voice',
    speechUnsupported: 'Speech playback is not supported in this browser.',
    speechVoices: (count: number) => `Detected voices: ${count}`,
    speechNoVoices: 'No browser voices detected. Install a Windows voice or wait a few seconds and try again.',
    speechVoice: (name: string) => `Active voice: ${name}`,
    speechVoicePicker: 'Choose a voice',
    speechBrowserDefault: 'Browser default voice',
    speechPreparing: (name: string) => `Preparing voice ${name}`,
    speechSpeaking: 'Speech playback in progress',
    speechDone: 'Speech playback finished',
    speechRetrying: 'The selected voice did not start. Retrying.',
    speechError: (message: string) => `Speech unavailable: ${message}`,
    fontStatus: (scale: number) => `Text size ${Math.round(scale * 100)} percent`,
    largeUiOn: 'Larger visuals enabled',
    largeUiOff: 'Larger visuals disabled',
    speechOn: 'Speech labels enabled',
    speechOff: 'Speech labels disabled',
  };
}
...
const ErrorAlert: React.FC<{ id?: string; message: string; solution?: string; onDismiss?: () => void }> = ({ id, message, solution, onDismiss }) => (
  <div
    id={id}
    role="alert"
    aria-live="assertive"
    aria-atomic="true"
    className="rounded-2xl bg-rose-50 dark:bg-rose-950/40 border border-rose-200 dark:border-rose-800/60 p-4 animate-in fade-in duration-300"
  >
    <div className="flex items-start gap-3">
      <div className="p-1.5 bg-rose-100 dark:bg-rose-900/40 rounded-lg shrink-0 mt-0.5">{renderDecorativeIcon(<AlertCircle className="w-4 h-4 text-rose-500" />)}</div>
      ...
      {onDismiss && (
        <button
          ...
          className="p-1 hover:bg-rose-100 dark:hover:bg-rose-900/40 rounded-lg text-rose-400 transition-colors shrink-0"
        >
          {renderDecorativeIcon(<X size={16} />)}
        </button>
      )}
    </div>
  </div>
...
const SuccessAlert: React.FC<{ id?: string; message: string; onDismiss?: () => void }> = ({ id, message, onDismiss }) => (
  <div
    id={id}
    role="status"
    aria-live="polite"
    aria-atomic="true"
    className="rounded-2xl bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800/60 p-4 animate-in fade-in duration-300"
  >
    <div className="flex items-start gap-3">
      <div className="p-2 bg-emerald-100 dark:bg-emerald-900/60 rounded-xl text-emerald-600 dark:text-emerald-300">
        {renderDecorativeIcon(<CheckCircle2 size={18} />)}
      </div>
      ...
      {onDismiss && (
        ...
        >
          {renderDecorativeIcon(<X size={18} />)}
        </button>
      )}
    </div>
  </div>
...
const InputField: React.FC<{
  ...
  describedBy?: string;
  autoComplete?: string;
}> = ({ id, label, type = 'text', value, onChange, placeholder, required, icon, disabled, description, describedBy, autoComplete }) => {
  const [showPwd, setShowPwd] = useState(false);
  const isPassword = type === 'password';
  const fallbackId = useId();
  const inputId = id ?? fallbackId;
  const descriptionId = description ? `${inputId}-description` : undefined;
  const describedByIds = [descriptionId, describedBy].filter(Boolean).join(' ') || undefined;
  return (
    <div className="space-y-1.5">
      <label htmlFor={inputId} className="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">{label}{required && <span className="text-rose-400 ml-0.5">*</span>}</label>
      <div className="relative">
        {icon && <div className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400">{renderDecorativeIcon(icon)}</div>}
        <input
          id={inputId}
        ...
        {isPassword && (
          ...
          >
            {showPwd ? renderDecorativeIcon(<EyeOff size={18} />) : renderDecorativeIcon(<Eye size={18} />)}
          </button>
        )}
      </div>
      {description && (
...
const SelectField: React.FC<{
  ...
  description?: string;
  describedBy?: string;
}> = ({ id, label, value, onChange, options, required, description, describedBy }) => {
  const fallbackId = useId();
  ...
  return (
  <div className="space-y-1.5">
    <label htmlFor={selectId} className="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">{label}{required && <span className="text-rose-400 ml-0.5">*</span>}</label>
    <div className="relative">
      <select
        id={selectId}
        value={value}
      ...
      <ChevronDown aria-hidden="true" size={16} className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" />
    </div>
    {description && (
      <p id={descriptionId} className="text-xs text-gray-500 dark:text-gray-400">
        {description}
...
const PrimaryButton: React.FC<{ children: React.ReactNode; onClick?: () => void; loading?: boolean; disabled?: boolean; type?: 'button' | 'submit'; className?: string; variant?: 'primary' | 'danger' | 'success' | 'outline' }> = 
  ...
  return (
    <button type={type} onClick={onClick} disabled={disabled || loading} aria-busy={loading || undefined} className={`${base} ${variants[variant]} ${className}`}>
...
const EmptyState: React.FC<{ icon: React.ReactNode; title: string; description: string; action?: React.ReactNode }> = ({ icon, title, description, action }) => (
  <div className="flex flex-col items-center justify-center py-16 px-8 text-center">
    <div className="p-4 bg-gray-100 dark:bg-gray-800 rounded-2xl text-gray-400 dark:text-gray-500 mb-4">{renderDecorativeIcon(icon)}</div>
...
const Card: React.FC<React.HTMLAttributes<HTMLDivElement> & { children: React.ReactNode; hover?: boolean }> = ({ children, className = '', hover, ...rest }) => (
  <div {...rest} className={`bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm ${hover ? 'hover:shadow-md hover:-translate-y-0.5 transition-all cursor-pointer' : ''} ${className}`}>
    {children}
  </div>
);
...
const AppLogoMark: React.FC<{ className?: string; alt?: string }> = ({
  ...
  <img
    src={appLogo}
    alt={alt}
    className={`a11y-logo shrink-0 object-contain ${className}`}
  />
);
...
const AppBrandLockup: React.FC<{
  ...
  <div className={`flex items-center gap-3 ${className}`}>
    <AppLogoMark className={logoClassName} alt="" />
    <span className={textClassName}>FinOps</span>
  </div>
);

const PageHeader: React.FC<{ id?: string; title: string; description?: string; actions?: React.ReactNode }> = ({ id, title, description, actions }) => (
  <header aria-labelledby={id} aria-describedby={description && id ? `${id}-description` : undefined} className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
    <div>
      <h1 id={id} className="text-2xl font-bold text-gray-900 dark:text-white">{title}</h1>
      {description && <p id={id ? `${id}-description` : undefined} className="text-sm text-gray-500 mt-1">{description}</p>}
    </div>
    {actions && <div className="flex items-center gap-2 shrink-0">{actions}</div>}
  </header>
);

const AccessibilityTooltipOverlay: React.FC<{ tooltip: TooltipState }> = ({ tooltip }) => {
  if (!tooltip.visible || !tooltip.text) return null;

  return (
    <div
      data-a11y-skip-tooltip="true"
      className="a11y-tooltip fixed z-[120] pointer-events-none rounded-xl bg-gray-950/95 px-3 py-2 text-xs font-medium text-white shadow-xl dark:bg-white dark:text-gray-900"
      style={{
        left: tooltip.x,
        top: tooltip.y,
        transform: tooltip.anchor === 'element' ? 'translate(-50%, -110%)' : undefined,
      }}
    >
      {tooltip.text}
    </div>
...
const AccessibilityPanel: React.FC<{
  ...
  const copy = getAccessibilityCopy(lang);
  const panelId = 'accessibility-panel';
  useEscapeToClose(isOpen, () => onOpenChange(false));

  return (
    <div className="fixed bottom-5 left-5 z-[110] flex flex-col items-start gap-3">
      {isOpen && (
        <Card
          id={panelId}
          data-a11y-skip-tooltip="true"
        ...
        >
          <div className="flex items-start justify-between gap-3">
            <div>
              <h2 id="accessibility-panel-title" className="text-sm font-bold text-gray-900 dark:text-white">
                {copy.panelTitle}
              </h2>
            ...
            <button
              ...
              className="rounded-xl border border-gray-200 px-2 py-1 text-xs font-semibold text-gray-600 transition-colors hover:bg-gray-50 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-gray-800"
            >
              {copy.closePanel}
            </button>
          </div>
          ...
          <div className="mt-4 space-y-4">
            <div>
              ...
              <div className="mt-2 grid grid-cols-3 gap-2">
                <button type="button" onClick={onDecreaseFont} className="rounded-xl border border-gray-200 px-3 py-2 text-sm font-bold text-gray-700 transition-colors hover:bg-gray-50 dark:border-gray-700 dark:text-gray-200 dark:hover:bg-gray-800">
                  {copy.smaller}
                </button>
                <button type="button" onClick={onResetFont} className="rounded-xl border border-gray-200 px-3 py-2 text-sm font-bold text-gray-700 transition-colors hover:bg-gray-50 dark:border-gray-700 dark:text-gray-200 dark:hover:bg-gray-800">
                  {copy.normal}
                </button>
                <button type="button" onClick={onIncreaseFont} className="rounded-xl border border-gray-200 px-3 py-2 text-sm font-bold text-gray-700 transition-colors hover:bg-gray-50 dark:border-gray-700 dark:text-gray-200 dark:hover:bg-gray-800">
                  {copy.larger}
                </button>
              </div>
            </div>
            ...
            <button
              ...
              className={`w-full rounded-2xl border px-4 py-3 text-left text-sm font-semibold transition-colors disabled:cursor-not-allowed disabled:opacity-60 ${
                speechEnabled
                  ? 'border-emerald-300 bg-emerald-50 text-emerald-700 dark:border-emerald-700 dark:bg-emerald-900/20 dark:text-emerald-300'
                  : 'border-gray-200 text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-200 dark:hover:bg-gray-800'
              }`}
            >
              <span className="block">{copy.speech}</span>
              {!speechSupported && (
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400">
              {copy.speechVoices(voiceCount)}
            </p>
            {voiceCount > 1 && (
              <label className="block">
                <span className="mb-1 block text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">
                  {copy.speechVoicePicker ?? 'Choose a voice'}
                </span>
                <select
                  value={selectedVoiceUri}
                  onChange={(event) => onSelectVoice(event.target.value)}
                  className="w-full rounded-xl border border-gray-200 bg-white px-3 py-2 text-sm text-gray-700 outline-none transition-colors focus:border-primary-500 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100"
                >
                  <option value="">{copy.speechBrowserDefault ?? 'Browser default voice'}</option>
            ...
            {activeVoiceLabel && (
              <p className="text-xs text-gray-500 dark:text-gray-400">
                {(copy.speechVoice?.(activeVoiceLabel)) ?? `Active voice: ${activeVoiceLabel}`}
              </p>
            )}
            {voiceCount === 0 && speechSupported && (
              <p className="text-xs text-amber-600 dark:text-amber-300">
                {copy.speechNoVoices}
              </p>
            )}
            {speechStatus && (
              <p role="status" aria-live="polite" className="text-xs text-primary-600 dark:text-primary-300">
                {speechStatus}
              </p>
            )}
            {speechSupported && (
              <button
                ...
                className="w-full rounded-2xl border border-gray-200 px-4 py-3 text-left text-sm font-semibold text-gray-700 transition-colors hover:bg-gray-50 dark:border-gray-700 dark:text-gray-200 dark:hover:bg-gray-800"
              >
                {copy.speechTest}
              </button>
            )}
      ...
      <button
        type="button"
        data-tooltip={isOpen ? copy.closePanel : copy.openPanel}
        aria-label={isOpen ? copy.closePanel : copy.openPanel}
        aria-expanded={isOpen}
        aria-controls={panelId}
        onClick={() => onOpenChange(!isOpen)}
        className="rounded-full bg-primary-600 px-4 py-3 text-sm font-bold text-white shadow-xl shadow-primary-600/25 transition-colors hover:bg-primary-700"
      >
        Aa
      </button>
    </div>
...
const LanguageSwitcher: React.FC<{
  ...
  const buttonId = `${switcherId}-button`;
  const listboxId = `${switcherId}-listbox`;
  const searchInputId = `${switcherId}-search`;
  ...
  useEscapeToClose(open, closeSwitcher);
  useEffect(() => {
    if (open) {
  ...
  return (
    <div className="relative">
      <button
        ...
        aria-label={compact ? t('lang.switcher.title', lang) : undefined}
      ...
      >
        {isLoading ? <Loader2 aria-hidden="true" size={18} className="animate-spin" /> : <Globe aria-hidden="true" size={18} />}
      ...
      {open && (
        <>
          <button type="button" className="fixed inset-0 z-40 cursor-default" onClick={closeSwitcher} aria-label="Close language selector" />
          <div
            id={listboxId}
            role="listbox"
          ...
          >
            <div className="p-2 border-b border-gray-100 dark:border-gray-800">
              <label htmlFor={searchInputId} className="sr-only">{t('lang.switcher.search', lang)}</label>
              <div className="relative">
                <Search aria-hidden="true" size={14} className="absolute start-3 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" />
                <input
                  id={searchInputId}
                  ref={searchInputRef}
                  value={query}
            ...
            {filteredOptions.map((l) => {
              ...
              >
                ...
                <span>{l.label}</span>
                {selected && <CheckCircle2 aria-hidden="true" size={14} className="ml-auto text-primary-500" />}
              </button>
            );})}
            ...
            {unavailableMessage && (
              <p aria-live="polite" className="px-4 py-2.5 text-[11px] border-t border-amber-200 dark:border-amber-900/50 bg-amber-50 dark:bg-amber-900/20 text-amber-700 dark:text-amber-300">
                {unavailableMessage}
              </p>
            )}
          </div>
...
const App: React.FC = () => {
  ...
  const [isLargeUi, setIsLargeUi] = useState<boolean>(() => getStoredBoolean(A11Y_LARGE_UI_STORAGE_KEY));
  const [speechEnabled, setSpeechEnabled] = useState<boolean>(() => getStoredBoolean(A11Y_SPEECH_STORAGE_KEY));
  const [selectedSpeechVoiceUri, setSelectedSpeechVoiceUri] = useState<string>(() => getStoredString(A11Y_SPEECH_VOICE_STORAGE_KEY));
  const [availableVoices, setAvailableVoices] = useState<SpeechSynthesisVoice[]>([]);
  const [isAccessibilityPanelOpen, setIsAccessibilityPanelOpen] = useState(false);
  const [a11yStatus, setA11yStatus] = useState('');
  const [speechStatus, setSpeechStatus] = useState('');
  const [activeSpeechVoiceLabel, setActiveSpeechVoiceLabel] = useState<string | null>(null);
  ...
  const sidebarId = 'app-sidebar-navigation';
  const notificationPanelId = 'app-notification-panel';
  const lastPersistedLangKey = useRef<string | null>(null);
  const tooltipTargetRef = useRef<HTMLElement | null>(null);
  const spokenTargetRef = useRef<HTMLElement | null>(null);
  const speechAttemptRef = useRef(0);
  const speechStartTimeoutRef = useRef<number | null>(null);
  useEscapeToClose(isNotificationPanelOpen, () => setIsNotificationPanelOpen(false));
  const speechSupported =
    typeof window !== 'undefined' &&
    'speechSynthesis' in window &&
    typeof SpeechSynthesisUtterance !== 'undefined';
  const accessibilityCopy = useMemo(() => getAccessibilityCopy(lang), [lang]);
  ...
  useEffect(() => {
    ...
    localStorage.setItem(A11Y_FONT_SCALE_STORAGE_KEY, String(fontScale));
  }, [fontScale]);

  useEffect(() => {
    document.documentElement.dataset.largeUi = isLargeUi ? 'true' : 'false';
    localStorage.setItem(A11Y_LARGE_UI_STORAGE_KEY, String(isLargeUi));
  }, [isLargeUi]);

  useEffect(() => {
    localStorage.setItem(A11Y_SPEECH_STORAGE_KEY, String(speechEnabled));
  ...
  useEffect(() => {
    localStorage.setItem(A11Y_SPEECH_VOICE_STORAGE_KEY, selectedSpeechVoiceUri);
  ...
  useEffect(() => {
    ...
    const updateVoices = () => {
      ...
      if (!nextVoices.length && retryTimer == null) {
  ...
  useEffect(() => {
    if (!selectedSpeechVoiceUri) return;
    const stillExists = availableVoices.some((voice) => voice.voiceURI === selectedSpeechVoiceUri);
    if (!stillExists) {
      setSelectedSpeechVoiceUri('');
      setActiveSpeechVoiceLabel(null);
    }
  }, [availableVoices, selectedSpeechVoiceUri]);
  ...
  const announceA11yStatus = useCallback((message: string) => {
    setA11yStatus('');
    window.setTimeout(() => setA11yStatus(message), 10);
  }, []);

  const clearSpeechStartTimeout = useCallback(() => {
  ...
  useEffect(() => clearSpeechStartTimeout, [clearSpeechStartTimeout]);

  const speakText = useCallback((text: string, options?: { force?: boolean; element?: HTMLElement | null }) => {
    const normalized = normalizeTooltipText(text);
    if (!normalized || !speechSupported || typeof SpeechSynthesisUtterance === 'undefined') return;
    if (!options?.force && !speechEnabled) return;
    if (options?.element && spokenTargetRef.current === options.element) return;

    const locale = localeForLang(lang);
    const voices = availableVoices.length ? availableVoices : window.speechSynthesis.getVoices();
    const localeLower = locale.toLowerCase();
    const primaryLang = localeLower.split('-')[0];
    const fallbackLang = primaryLang === 'fr' ? 'fr-FR' : primaryLang === 'ar' ? 'ar' : 'en-US';
    const selectedVoice = selectedSpeechVoiceUri
      ? voices.find((voice) => voice.voiceURI === selectedSpeechVoiceUri) ?? null
      : null;
    const preferredVoices = [
      selectedVoice,
      voices.find((voice) => voice.lang.toLowerCase() === localeLower) ?? null,
      voices.find((voice) => voice.lang.toLowerCase().startsWith(primaryLang)) ?? null,
      voices.find((voice) => voice.default) ?? null,
      voices[0] ?? null,
    ].filter((voice, index, list): voice is SpeechSynthesisVoice => {
      if (!voice) return false;
      const identity = `${voice.voiceURI}:${voice.lang}:${voice.name}`;
      return list.findIndex((item) => item && `${item.voiceURI}:${item.lang}:${item.name}` === identity) === index;
    });

    const candidates: Array<{ voice: SpeechSynthesisVoice | null; lang: string; label: string }> = [
      ...preferredVoices.map((voice) => ({
        voice,
        lang: voice.lang || fallbackLang,
        label: voice.name || voice.lang || 'browser voice',
      })),
      { voice: null, lang: fallbackLang, label: 'browser default' },
    ];

    const nextAttemptId = speechAttemptRef.current + 1;
    speechAttemptRef.current = nextAttemptId;
    spokenTargetRef.current = options?.element ?? null;
    clearSpeechStartTimeout();
    setSpeechStatus('');

    const runCandidate = (candidateIndex: number) => {
      const candidate = candidates[candidateIndex];
      if (!candidate || speechAttemptRef.current !== nextAttemptId) {
        setSpeechStatus((accessibilityCopy.speechError?.('no voice could start')) ?? 'Speech unavailable: no voice could start');
        return;
      }
      ...
      setActiveSpeechVoiceLabel(candidate.label);
      setSpeechStatus((accessibilityCopy.speechPreparing?.(candidate.label)) ?? `Preparing voice ${candidate.label}`);

      utterance.onstart = () => {
        if (speechAttemptRef.current !== nextAttemptId) return;
        hasStarted = true;
        clearSpeechStartTimeout();
        setSpeechStatus(accessibilityCopy.speechSpeaking ?? 'Speech playback in progress');
      };
      ...
      utterance.onend = () => {
        ...
        setSpeechStatus(accessibilityCopy.speechDone ?? 'Speech playback finished');
      };

      utterance.onerror = (event) => {
        if (speechAttemptRef.current !== nextAttemptId) return;
        clearSpeechStartTimeout();
        const errorCode = normalizeTooltipText((event as SpeechSynthesisErrorEvent).error) || 'browser error';
        if (candidateIndex < candidates.length - 1) {
          setSpeechStatus(accessibilityCopy.speechRetrying ?? 'The selected voice did not start. Retrying.');
          window.setTimeout(() => runCandidate(candidateIndex + 1), 60);
          return;
        }
        spokenTargetRef.current = null;
        setSpeechStatus((accessibilityCopy.speechError?.(errorCode)) ?? `Speech unavailable: ${errorCode}`);
      };
      ...
      speechStartTimeoutRef.current = window.setTimeout(() => {
        if (speechAttemptRef.current !== nextAttemptId || hasStarted) return;
        window.speechSynthesis.cancel();
        if (candidateIndex < candidates.length - 1) {
          setSpeechStatus(accessibilityCopy.speechRetrying ?? 'The selected voice did not start. Retrying.');
          runCandidate(candidateIndex + 1);
          return;
        }
        spokenTargetRef.current = null;
        setSpeechStatus((accessibilityCopy.speechError?.('playback did not start')) ?? 'Speech unavailable: playback did not start');
      }, 1500);
      ...
      try {
        ...
        spokenTargetRef.current = null;
        setSpeechStatus((accessibilityCopy.speechError?.(message)) ?? `Speech unavailable: ${message}`);
      }
    };

    runCandidate(0);
  }, [accessibilityCopy, availableVoices, clearSpeechStartTimeout, lang, selectedSpeechVoiceUri, speechEnabled, speechSupported]);

  const speakTooltipText = useCallback((text: string, element: HTMLElement) => {
  ...
  useEffect(() => {
    ...
    const showTooltip = (
      element: HTMLElement,
      nextTooltip: Pick<TooltipState, 'x' | 'y' | 'anchor'>,
    ) => {
      const text = extractInteractiveTooltipText(element);
      if (!text) {
        hideTooltip();
        return;
      }
      ...
      setTooltip({
        ...
        x: nextTooltip.x,
        y: nextTooltip.y,
        anchor: nextTooltip.anchor,
      });
      speakTooltipText(text, element);
    };

    const handleMouseOver = (event: MouseEvent) => {
      const interactive = getInteractiveTooltipTarget(event.target);
      if (!interactive) {
        hideTooltip();
        return;
      }
      if (tooltipTargetRef.current === interactive) return;
  ...
  const handleDecreaseFont = () => {
    setFontScale((prev) => {
      const next = prev <= 0.9 ? 0.9 : Number((prev - 0.1).toFixed(2));
      announceA11yStatus(accessibilityCopy.fontStatus(next));
      return next;
    });
  };
  ...
  const handleResetFont = () => {
    setFontScale(1);
    announceA11yStatus(accessibilityCopy.fontStatus(1));
  };

  const handleIncreaseFont = () => {
    setFontScale((prev) => {
      const next = prev >= 1.2 ? 1.2 : Number((prev + 0.1).toFixed(2));
      announceA11yStatus(accessibilityCopy.fontStatus(next));
      return next;
    });
  };

  const handleToggleLargeUi = () => {
    setIsLargeUi((prev) => {
      const next = !prev;
      announceA11yStatus(next ? accessibilityCopy.largeUiOn : accessibilityCopy.largeUiOff);
      return next;
    });
  };
  ...
  const handleToggleSpeech = () => {
    setSpeechEnabled((prev) => {
      const next = !prev;
      announceA11yStatus(next ? accessibilityCopy.speechOn : accessibilityCopy.speechOff);
      return next;
    });
    if (!speechEnabled && speechSupported) {
      speakText(accessibilityCopy.speechOn, { force: true });
    }
  };

  const handleSelectSpeechVoice = (voiceUri: string) => {
    setSelectedSpeechVoiceUri(voiceUri);
    const nextVoice = availableVoices.find((voice) => voice.voiceURI === voiceUri);
    if (nextVoice) {
      setActiveSpeechVoiceLabel(nextVoice.name || nextVoice.lang || 'browser voice');
    } else {
      setActiveSpeechVoiceLabel(null);
    }
  };
  ...
  const handleTestSpeech = () => {
    speakText(getSpeechTestSentence(lang), { force: true });
  };

  const renderAccessibilityChrome = () => (
    <>
      <AccessibilityPanel
        lang={lang}
        ...
        largeUi={isLargeUi}
        onToggleLargeUi={handleToggleLargeUi}
        ...
        onSelectVoice={handleSelectSpeechVoice}
        speechStatus={speechStatus}
        activeVoiceLabel={activeSpeechVoiceLabel}
      />
      <AccessibilityTooltipOverlay tooltip={tooltip} />
  ...
  if (!user) {
    if (showRegisterBusiness) {
      return (
        <>
          <BusinessRegistrationView lang={lang} setLang={pickUiLanguage} languageOptions={languageOptions} translationUnavailableMessage={translationUnavailableMessage} onSuccess={() => { setShowRegisterBusiness(false); setAuthMode('login'); setShowAuth(true); }} onCancel={() => setShowRegisterBusiness(false)} />
          {renderAccessibilityChrome()}
        </>
      );
    }
    ...
    return (
      <>
        <LandingPageView lang={lang} setLang={pickUiLanguage} languageOptions={languageOptions} translationUnavailableMessage={translationUnavailableMessage} isDarkMode={isDarkMode} onToggleTheme={handleToggleTheme} onEnter={() => { setAuthMode('login'); setShowAuth(true); }} onGetStarted={() => setShowRegisterBusiness(true)} />
        {renderAccessibilityChrome()}
      </>
    );
  }
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside id={sidebarId} aria-label="Primary navigation" className={`${isSidebarOpen ? 'w-64' : 'w-[72px]'} bg-white dark:bg-gray-900 border-e border-gray-200 dark:border-gray-800 transition-all duration-300 flex flex-col fixed h-full z-50`}>
        ...
        <nav aria-label="Primary" className="flex-1 p-3 space-y-0.5 overflow-y-auto overflow-x-hidden">
          {navItems.map(item => (
        ...
        <div className="p-3 border-t border-gray-100 dark:border-gray-800 space-y-0.5">
          ...
          <button type="button" onClick={handleLogout} aria-label={t('nav.logout', lang)} className={`w-full flex items-center gap-3 px-3 py-2.5 text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-950/30 rounded-xl transition-colors text-sm font-medium ${!isSidebarOpen && 'justify-center'}`}>
      ...
      <main className={`relative flex-1 transition-all duration-300 ${isSidebarOpen ? 'ms-64' : 'ms-[72px]'}`}>
        <header className="sticky top-0 z-40 bg-white/80 dark:bg-gray-950/80 backdrop-blur-xl border-b border-gray-200 dark:border-gray-800 h-16 flex items-center justify-between px-6">
          <div className="flex items-center gap-3">
            <button type="button" onClick={() => setIsSidebarOpen(!isSidebarOpen)} aria-expanded={isSidebarOpen} aria-controls={sidebarId} aria-label={isSidebarOpen ? 'Collapse sidebar navigation' : 'Expand sidebar navigation'} className="p-2 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-xl text-gray-500 dark:text-gray-400 transition-colors">
              {isSidebarOpen ? <X aria-hidden="true" size={18} /> : <Menu aria-hidden="true" size={18} />}
            </button>
            ...
            {tenant && (
                ...
                <Building2 aria-hidden="true" size={14} className="text-gray-400" />
                <span className="text-xs font-semibold text-gray-600 dark:text-gray-300">{tenant.name}</span>
              </div>
            )}
            {companyActionError && (
              <span role="alert" className="hidden lg:inline text-xs text-rose-500">{companyActionError}</span>
            )}
          </div>
          <div className="flex items-center gap-3">
            <LanguageSwitcher lang={lang} setLang={pickUiLanguage} options={languageOptions} compact isLoading={isTranslatingUi} unavailableMessage={translationUnavailableMessage} />
            <button
              ...
              aria-label={`${t('notif.title', lang)}${unreadNotificationsCount > 0 ? ` (${unreadNotificationsCount} unread)` : ''}`}
            ...
            >
              ...
              {unreadNotificationsCount > 0 && (
                <span aria-hidden="true" className="absolute -top-1 -right-1 min-w-[18px] h-[18px] px-1 rounded-full bg-rose-500 text-white text-[10px] font-bold flex items-center justify-center">
                  {unreadNotificationsCount > 99 ? '99+' : unreadNotificationsCount}
                </span>
              )}
            </button>
            <button type="button" onClick={handleToggleTheme} aria-label={isDarkMode ? 'Switch to light mode' : 'Switch to dark mode'} className="p-2 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-xl transition-colors text-gray-500 dark:text-gray-400">
              {isDarkMode ? <Sun aria-hidden="true" size={18} /> : <Moon aria-hidden="true" size={18} />}
            </button>
            <div className="h-6 w-px bg-gray-200 dark:bg-gray-800"></div>
            <div className="flex items-center gap-3">
              <div className="a11y-avatar w-8 h-8 rounded-full bg-primary-600 text-white flex items-center justify-center text-xs font-bold shadow-sm">
        ...
        {isNotificationPanelOpen && (
          <div id={notificationPanelId} className="absolute right-6 top-20 z-40 w-full max-w-md">
            <Card className="p-4 shadow-2xl">
              <section role="region" aria-labelledby="notification-panel-title">
              <div className="flex items-center justify-between mb-3">
                <h3 id="notification-panel-title" className="font-bold text-sm text-gray-900 dark:text-white">{t('notif.title', lang)}</h3>
                <button
                  type="button"
                  ...
                  {notifications.map((n) => (
                    ...
                    >
                      <div className="flex items-start justify-between gap-2">
                        ...
                        {!n.readAt && <span aria-hidden="true" className="w-2 h-2 rounded-full bg-rose-500 mt-1 shrink-0" />}
                      </div>
...
const AIAssistantWidget: React.FC<{ lang: UiLang }> = ({ lang }) => {
  ...
  return (
    <div className="fixed bottom-5 end-5 z-[70]">
      {open && (
        <Card id={panelId} className="w-[360px] max-w-[calc(100vw-2rem)] p-4 mb-3 shadow-2xl max-h-[min(70vh,520px)] flex flex-col" role="dialog" aria-modal="false" aria-labelledby="ai-assistant-title">
          <div className="flex items-center justify-between mb-3 shrink-0">
            <h3 id="ai-assistant-title" className="font-bold text-gray-900 dark:text-white text-sm">{t('ai.chat.title', lang)}</h3>
            <button
              type="button"
          ...
          <div className="flex gap-2 mb-3 shrink-0">
            <label htmlFor="ai-assistant-message" className="sr-only">{t('ai.chat.placeholder', lang)}</label>
            <input
          ...
          <div className="overflow-y-auto min-h-[48px] flex-1 pe-1" aria-live="polite">
            {reply ?
      ...
      <button
        ...
        aria-expanded={open}
        aria-controls={panelId}
        aria-label={open ? 'Close AI assistant' : t('ai.chat.openTitle', lang)}
      ...
      <Card className="max-w-md w-full p-8">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-1">
            ...
            <button type="button" onClick={onCancel} aria-label="Close business registration" className="p-2 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-xl text-gray-400 transition-colors"><X aria-hidden="true" size={18} /></button>
          </div>
        </div>
...
const ChangePasswordModal: React.FC<{
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 flex items-center justify-center p-6" role="dialog" aria-modal="true" aria-labelledby="change-password-title" aria-describedby="change-password-description">
      <Card className="max-w-md w-full p-8">
        ...
        <div className="flex justify-center mb-5">
          <div className="w-14 h-14 bg-amber-100 dark:bg-amber-900/30 rounded-2xl flex items-center justify-center"><ShieldCheck aria-hidden="true" size={28} className="text-amber-500" /></div>
        </div>
        <h1 id="change-password-title" className="text-xl font-bold text-gray-900 dark:text-white mb-1 text-center">{t('changepwd.title', lang)}</h1>
        <p id="change-password-description" className="text-sm text-gray-500 mb-6 text-center">{t('changepwd.desc', lang)}</p>
        <form onSubmit={handleSubmit} className="space-y-4">
          <InputField label={t('changepwd.current', lang)} type="password" value={current} onChange={setCurrent} required autoComplete="current-password" />
          <div>
            <InputField label={t('changepwd.new', lang)} type="password" value={newPass} onChange={setNewPass} required placeholder={t('changepwd.newPlaceholder', lang)} describedBy={rulesId} autoComplete="new-password" />
            {newPass && (
          ...
          <InputField label={t('changepwd.confirm', lang)} type="password" value={confirm} onChange={setConfirm} required autoComplete="new-password" />
          <div id={rulesId} className="bg-gray-50 dark:bg-gray-800/60 rounded-xl p-3 border border-gray-100 dark:border-gray-700">
            <p className="text-xs text-gray-500 space-y-1">
              <span className={`flex items-center gap-2 ${newPass.length >= 8 ? 'text-emerald-500' : ''}`}><CircleDot aria-hidden="true" size={10} /> {t('changepwd.rule1', lang)}</span>
              <span className={`flex items-center gap-2 ${/[A-Z]/.test(newPass) ? 'text-emerald-500' : ''}`}><CircleDot aria-hidden="true" size={10} /> {t('changepwd.rule2', lang)}</span>
              <span className={`flex items-center gap-2 ${/[a-z]/.test(newPass) ? 'text-emerald-500' : ''}`}><CircleDot aria-hidden="true" size={10} /> {t('changepwd.rule3', lang)}</span>
              <span className={`flex items-center gap-2 ${/\d/.test(newPass) ? 'text-emerald-500' : ''}`}><CircleDot aria-hidden="true" size={10} /> {t('changepwd.rule4', lang)}</span>
            </p>
          </div>
          {error && <ErrorAlert message={error} solution={errorSolution} onDismiss={() => { setError(''); setErrorSolution(undefined); }} />}
...
const LandingPageView: React.FC<{ lang: UiLang; setLang: (l: UiLang) => void; languageOptions: LanguageOption[]; translationUnavailableMessage: string | null; isDarkMode: boolean; onToggleTheme: () => void; onEnter: () => void; onGetStarted: () => void }> = ({ lang, setLang, languageOptions, translationUnavailableMessage, isDarkMode, onToggleTheme, onEnter, onGetStarted }) => (
  <main className="min-h-screen bg-white dark:bg-gray-950 transition-colors overflow-hidden" aria-labelledby="landing-hero-title">
    <nav aria-label="Landing page" className="fixed top-0 w-full z-50 bg-white/80 dark:bg-gray-950/80 backdrop-blur-xl border-b border-gray-200 dark:border-gray-800">
      <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <LanguageSwitcher lang={lang} setLang={setLang} options={languageOptions} unavailableMessage={translationUnavailableMessage} compact />
          <button type="button" onClick={onToggleTheme} aria-label={isDarkMode ? 'Switch to light mode' : 'Switch to dark mode'} className="p-2 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-xl text-gray-500 dark:text-gray-400 transition-colors">
            {isDarkMode ? <Sun aria-hidden="true" size={18} /> : <Moon aria-hidden="true" size={18} />}
          </button>
    ...
    <section className="pt-36 pb-20 px-6 max-w-5xl mx-auto text-center relative" aria-labelledby="landing-hero-title">
      ...
      <h1 id="landing-hero-title" className="text-5xl md:text-7xl font-bold tracking-tight mt-6 mb-6 text-gray-900 dark:text-white leading-[1.1]">
    ...
    <section id="features" aria-labelledby="landing-features-title" className="py-16 px-6 max-w-6xl mx-auto">
      <h2 id="landing-features-title" className="sr-only">{t('landing.features', lang)}</h2>
    ...
    <section id="stats" aria-labelledby="landing-stats-title" className="py-16 bg-primary-600 text-white relative overflow-hidden">
      <h2 id="landing-stats-title" className="sr-only">{t('landing.performance', lang)}</h2>
...
const FeatureCard: React.FC<{ icon: React.ReactNode; title: string; desc: string }> = ({ icon, title, desc }) => {
  const titleId = useId();
  return (
    <Card hover className="p-8 group" aria-labelledby={titleId}>
      <div className="w-12 h-12 bg-gray-50 dark:bg-gray-800 rounded-xl flex items-center justify-center mb-5 group-hover:bg-primary-600 group-hover:text-white transition-all">
        {renderDecorativeIcon(React.cloneElement(icon as React.ReactElement<any>, { size: 24 }))}
      </div>
      <h3 id={titleId} className="text-lg font-bold mb-2 text-gray-900 dark:text-white">{title}</h3>
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-gray-950 flex items-center justify-center p-6">
        <Card className="max-w-md w-full p-8">
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex items-center gap-1">
              <LanguageSwitcher lang={lang} setLang={setLang} options={languageOptions} unavailableMessage={translationUnavailableMessage} compact />
              <button type="button" onClick={() => { setShowForgotPassword(false); setForgotError(''); setForgotEmail(''); setForgotResult(null); }} aria-label="Close forgot password form" className="p-2 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-xl text-gray-400 transition-colors"><X aria-hidden="true" size={18} /></button>
            </div>
          </div>
        ...
        <Card className="max-w-md w-full p-8">
          <div className="flex items-center justify-between mb-6">
            ...
            <button
              ...
              aria-label="Close two-factor sign-in"
      ...
      <Card className="w-full max-w-lg overflow-hidden">
        <div className="p-8">
          ...
          <div className="flex justify-between items-start mb-6">
            ...
            <div className="flex items-center gap-1">
              <LanguageSwitcher lang={lang} setLang={setLang} options={languageOptions} unavailableMessage={translationUnavailableMessage} compact />
              <button type="button" onClick={onCancel} aria-label="Close sign in" className="p-2 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-xl text-gray-400 transition-colors"><X aria-hidden="true" size={18} /></button>
            </div>
          </div>
          ...
          {useRealBackend && showRolesInfo && (
            <div className="space-y-2 mb-6">
              ...
              {ROLE_DESCRIPTIONS.map(rd => (
                <button type="button" key={rd.id} onClick={() => handleSelectRole(rd.id)}
          ...
          {(!useRealBackend || !showRolesInfo) && (
            <div className="space-y-4">
              ...
              <div className="text-center pt-2">
                {useRealBackend ? (
                  <button type="button" onClick={onShowRegisterBusiness} className="text-xs font-medium text-gray-400 hover:text-primary-600 transition-colors">
                    {t('auth.noAccount', lang)} <span className="text-primary-600 font-semibold">{t('auth.registerBusiness', lang)}</span>
                  </button>
                ) : (
                  <button type="button" onClick={() => setMode(mode === 'login' ? 'register' : 'login')} className="text-xs font-medium text-gray-400 hover:text-primary-600 transition-colors">
        ...
        {!useRealBackend && (
          <div className="px-8 py-4 bg-gray-50 dark:bg-gray-800/40 border-t border-gray-100 dark:border-gray-800">
            ...
            <div className="flex gap-2">
              <button type="button" onClick={() => { setEmail('admin@devsphere.com'); setPassword('password'); }} className="text-xs px-3 py-1.5 rounded-lg bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-300 hover:border-primary-500 transition-colors font-medium">Admin</button>
              <button type="button" onClick={() => { setEmail('jane@acme.com'); setPassword('password'); }} className="text-xs px-3 py-1.5 rounded-lg bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-300 hover:border-primary-500 transition-colors font-medium">Client</button>
            </div>
          </div>
        )}
      </Card>
...
const NavItem: React.FC<{ icon: React.ReactNode; label: string; active: boolean; onClick: () => void; expanded: boolean }> = ({ icon, label, active, onClick, expanded }) => (
  <button type="button" onClick={onClick} aria-current={active ? 'page' : undefined} aria-label={!expanded ? label : undefined} title={!expanded ? label : undefined} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all text-sm ${
    ...
    <div className="shrink-0">{renderDecorativeIcon(icon)}</div>
...
const StatCard: React.FC<{
  ...
  return (
    <Card className="p-6" aria-labelledby={titleId}>
      <div className="flex items-center justify-between mb-4">
        <div className="p-2.5 bg-gray-50 dark:bg-gray-800 rounded-xl text-primary-600">{renderDecorativeIcon(icon)}</div>
        {change !== undefined && change !== '' && (
          <Badge variant={isPositive ? 'success' : 'danger'}>{change}</Badge>
        )}
      </div>
      <p id={titleId} className="text-xs font-medium text-gray-500 uppercase tracking-wider mb-1">{title}</p>
...
const AdminRegistrationRequestsView: React.FC<{ lang: UiLang }> = ({ lang }) => {
  ...
  return (
    <section className="space-y-6" aria-labelledby="admin-requests-title">
      <PageHeader id="admin-requests-title" title={t('admin.requests.title', lang)} description={t('admin.requests.desc', lang)}
        actions={
              ...
              <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                ...
                {req.status === 'pending' && (
                  <div className="flex flex-col gap-2 lg:w-80 shrink-0">
                    <input
                      aria-label={t('admin.requests.rejectReason', lang)}
                      aria-describedby={`registration-reject-help-${req.id}`}
                    ...
                    />
                    <p id={`registration-reject-help-${req.id}`} className="text-xs text-gray-500 dark:text-gray-400">
                      {t('admin.requests.rejectMinLength', lang)}
                    </p>
                    <div className="flex gap-2">
              ...
              <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                ...
                {req.status === 'pending' && (
                  <div className="flex flex-col gap-2 lg:w-80 shrink-0">
                    <input
                      aria-label={t('admin.requests.rejectReason', lang)}
                      aria-describedby={`employee-reject-help-${req.id}`}
                      placeholder={t('admin.requests.rejectReason', lang)}
                      value={rejectReason[req.id] || ''}
                      onChange={(e) => setRejectReason((prev) => ({ ...prev, [req.id]: e.target.value }))}
                      className="px-3.5 py-2.5 rounded-xl bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-sm outline-none focus:border-primary-500 transition-colors"
                    />
                    <p id={`employee-reject-help-${req.id}`} className="text-xs text-gray-500 dark:text-gray-400">
                      {t('admin.requests.rejectMinLength', lang)}
                    </p>
                    <div className="flex gap-2">
                      <PrimaryButton onClick={() => handleAccept(req.id)} disabled={!!processing} variant="success" className="flex-1 py-2.5 text-xs">
...
const EmployeeCompanyJoinView: React.FC<{ lang: UiLang }> = ({ lang }) => {
  ...
  return (
    <section className="space-y-6" aria-labelledby="join-company-title">
      <PageHeader
            ...
            <Card key={req.id} className="p-4">
              <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                ...
                <div className="lg:w-80 space-y-2">
                  <input
                    aria-label={t('join.manager.rejectPh', lang)}
                    aria-describedby={`join-request-reject-help-${req.id}`}
                  ...
                  />
                  <p id={`join-request-reject-help-${req.id}`} className="text-xs text-gray-500 dark:text-gray-400">
                    {t('admin.requests.rejectMinLength', lang)}
                  </p>
                  <div className="flex gap-2">
...
const AdminEmployeesView: React.FC<{ companyId: string; lang: UiLang }> = ({ companyId, lang }) => {
  ...
  return (
    <section className="space-y-6" aria-labelledby="employees-page-title">
      <PageHeader id="employees-page-title" title={t('emp.title', lang)} description={t('emp.desc', lang)}
        actions={
          <div className="flex gap-2">
      ...
      {loading ? (
        ...
        <Card className="overflow-hidden">
          <table className="w-full text-left">
            <caption className="sr-only">{t('emp.title', lang)}</caption>
            <thead className="bg-gray-50 dark:bg-gray-800/50 border-b border-gray-100 dark:border-gray-800">
              <tr>
                <th scope="col" className="px-6 py-3.5 text-xs font-semibold text-gray-500 uppercase tracking-wider">{t('emp.colEmployee', lang)}</th>
                <th scope="col" className="px-6 py-3.5 text-xs font-semibold text-gray-500 uppercase tracking-wider">{t('common.email', lang)}</th>
                <th scope="col" className="px-6 py-3.5 text-xs font-semibold text-gray-500 uppercase tracking-wider">{t('emp.role', lang)}</th>
              </tr>
            </thead>
              ...
              {employees.map(emp => (
                <tr key={emp.id} className="hover:bg-gray-50/50 dark:hover:bg-gray-800/30 transition-colors">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <div className="a11y-avatar w-9 h-9 rounded-full bg-primary-100 dark:bg-primary-900/30 text-primary-600 flex items-center justify-center text-xs font-bold">
...
const AdminInvoicesView: React.FC<{ user: User; lang: UiLang }> = ({ user, lang }) => {
  ...
  const closePaymentSuggestion = useCallback(() => {
    if (!paymentSuggestionLoading) {
  ...
  useEscapeToClose(intakeOpen, closeIntake);
  useEscapeToClose(paymentSuggestionOpen, closePaymentSuggestion);
  useEscapeToClose(modalOpen, closeInvoiceModal);
  ...
  return (
    <section className="space-y-6" aria-labelledby="invoices-page-title">
      <PageHeader
        ...
        {loading ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left min-w-[720px]">
              <caption className="sr-only">{t('page.invoices.title', lang)}</caption>
              <thead className="bg-gray-50 dark:bg-gray-800/50 border-b border-gray-100 dark:border-gray-800">
                <tr>
                  <th scope="col" className="px-6 py-3.5 text-xs font-semibold text-gray-500 uppercase tracking-wider">{t('page.invoices.document', lang)}</th>
                  <th scope="col" className="px-6 py-3.5 text-xs font-semibold text-gray-500 uppercase tracking-wider">{t('page.invoices.client', lang)}</th>
                  <th scope="col" className="px-6 py-3.5 text-xs font-semibold text-gray-500 uppercase tracking-wider text-right">{t('page.invoices.amount', lang)}</th>
                  <th scope="col" className="px-6 py-3.5 text-xs font-semibold text-gray-500 uppercase tracking-wider text-right">{t('page.invoices.status', lang)}</th>
                  <th scope="col" className="px-6 py-3.5 text-xs font-semibold text-gray-500 uppercase tracking-wider text-right">{t('page.invoices.actions', lang)}</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                {data.map((inv) => (
      ...
      {intakeOpen && (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="invoice-intake-title"
          aria-describedby="invoice-intake-description"
        >
          <Card className="w-full max-w-5xl p-6 shadow-2xl max-h-[92vh] overflow-y-auto">
            <div className="flex items-start justify-between gap-3 mb-5">
              <div>
                <h3 id="invoice-intake-title" className="font-bold text-lg text-gray-900 dark:text-white">
                  Smart Invoice / Receipt Intake
                </h3>
                <p id="invoice-intake-description" className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                  {lang === 'fr'
              ...
              <button
                ...
                onClick={closeIntake}
                aria-label="Close smart intake dialog"
              >
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
              <div className="space-y-4">
                ...
                />
                <div className="space-y-1.5">
                  <label htmlFor="invoice-intake-source-text" className="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                    {lang === 'fr' ? 'Texte du document' : 'Document text'}
                  </label>
                  <textarea
                    id="invoice-intake-source-text"
                    aria-describedby="invoice-intake-source-help"
                  ...
                  />
                  <p id="invoice-intake-source-help" className="text-xs text-gray-500 dark:text-gray-400">
                    {lang === 'fr' ? 'Collez le contenu brut du document pour laisser l’IA proposer un brouillon.' : 'Paste the raw document content so AI can propose a draft.'}
                  </p>
                </div>
                <div className="flex gap-2">
      ...
      {paymentSuggestionOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" role="dialog" aria-modal="true" aria-labelledby="invoice-payment-suggestion-title" aria-describedby="invoice-payment-suggestion-description">
          <div className="w-full max-w-2xl rounded-[28px] bg-white dark:bg-gray-950 shadow-2xl overflow-hidden">
            <div className="flex items-start justify-between gap-4 border-b border-gray-200 dark:border-gray-800 px-6 py-5">
              <div>
                <h3 id="invoice-payment-suggestion-title" className="text-lg font-semibold text-gray-900 dark:text-white">{t('page.invoices.suggestionTitle', lang)}</h3>
                <p id="invoice-payment-suggestion-description" className="text-sm text-gray-500">{paymentSuggestionInvoice?.number}</p>
              </div>
              <button type="button" onClick={closePaymentSuggestion} aria-label="Close payment suggestion dialog" className="rounded-full p-2 text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-800 dark:text-gray-400">
                <X aria-hidden="true" size={18} />
              </button>
            </div>
            <div className="p-6 space-y-5">
              {paymentSuggestionLoading ? (
                <div role="status" aria-live="polite" className="py-16 flex justify-center">
                  <Loader2 aria-hidden="true" className="animate-spin text-primary-600" />
                </div>
            ...
            <div className="flex justify-end gap-3 border-t border-gray-200 dark:border-gray-800 px-6 py-4">
              <PrimaryButton type="button" variant="outline" onClick={closePaymentSuggestion}>
        ...
        >
          <Card className="w-full max-w-lg p-6 shadow-2xl">
            <div className="flex items-start justify-between gap-3 mb-4">
              <h3 id="invoice-form-title" className="font-bold text-lg text-gray-900 dark:text-white">
            ...
            <form noValidate onSubmit={(e) => void submitInvoice(e)} className="space-y-4">
              ...
              <div className="flex gap-2 pt-2">
                ...
                <PrimaryButton type="button" variant="outline" className="py-2.5 text-sm" onClick={closeInvoiceModal} disabled={saving}>
                  {t('page.invoices.cancel', lang)}
                </PrimaryButton>
              </div>
            </form>
...
const ExpenseForecastPanel: React.FC<{ lang: UiLang; companyId: string; onClose: () => void }> = ({ lang, companyId, onClose }) => {
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="expense-forecast-title"
      aria-describedby="expense-forecast-description"
    >
      <Card className="w-full max-w-2xl p-6 shadow-2xl max-h-[90vh] overflow-y-auto">
        <div className="flex items-start justify-between gap-3 mb-6">
          <div>
            <h3 id="expense-forecast-title" className="font-bold text-xl text-gray-900 dark:text-white">
              {t('ai.expense.forecast.title', lang)}
            </h3>
            <p id="expense-forecast-description" className="text-sm text-gray-500 mt-1">
          ...
          <button
            ...
            className="p-1.5 rounded-lg text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors shrink-0"
            onClick={onClose}
            aria-label="Close expense forecast dialog"
          >
            <X aria-hidden="true" size={20} />
          </button>
        </div>
        ...
        {loading ? (
          <div role="status" aria-live="polite" className="flex justify-center py-12">
            <Loader2 aria-hidden="true" className="animate-spin text-primary-600" size={32} />
...
const ExpenseAlertsPanel: React.FC<{ lang: UiLang; companyId: string; onClose: () => void }> = ({ lang, companyId, onClose }) => {
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="expense-alerts-title"
      aria-describedby="expense-alerts-description"
    >
      <Card className="w-full max-w-4xl p-6 shadow-2xl max-h-[90vh] overflow-y-auto">
        <div className="flex items-start justify-between gap-3 mb-6">
          <div>
            <h3 id="expense-alerts-title" className="font-bold text-xl text-gray-900 dark:text-white">
              {t('ai.alerts.title', lang)}
            </h3>
            <p id="expense-alerts-description" className="text-sm text-gray-500 mt-1">
          ...
          <button
            ...
            className="p-1.5 rounded-lg text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors shrink-0"
            onClick={onClose}
            aria-label="Close expense alerts dialog"
          >
            <X aria-hidden="true" size={20} />
          </button>
        </div>
...
const AdminExpensesView: React.FC<{ user: User; lang: UiLang }> = ({ user, lang }) => {
  ...
  useEscapeToClose(modalOpen, closeExpenseModal);
  ...
  return (
    <section className="space-y-6" aria-labelledby="expenses-page-title">
      <PageHeader
      ...
      {modalOpen && (
        ...
        >
          <Card className="w-full max-w-md p-6 shadow-2xl">
            <div className="flex items-start justify-between gap-3 mb-4">
              <h3 id="expense-form-title" className="font-bold text-lg text-gray-900 dark:text-white">
            ...
            <form noValidate onSubmit={(e) => void submitExpense(e)} className="space-y-4">
              ...
              <div className="flex gap-2 pt-2">
                ...
                <PrimaryButton type="button" variant="outline" className="py-2.5 text-sm" onClick={closeExpenseModal} disabled={saving}>
                  {t('page.invoices.cancel', lang)}
                </PrimaryButton>
              </div>
            </form>
...
const AdminClientsView: React.FC<{ user: User; lang: UiLang }> = ({ user, lang }) => {
  ...
  useEscapeToClose(modalOpen, closeClientModal);
  ...
  return (
    <section className="space-y-6" aria-labelledby="clients-page-title">
      <PageHeader
      ...
      {modalOpen && (
        ...
        >
          <Card className="w-full max-w-md p-6 shadow-2xl">
            <div className="flex items-start justify-between gap-3 mb-4">
              <h3 id="client-form-title" className="font-bold text-lg text-gray-900 dark:text-white">
            ...
            <form noValidate onSubmit={(e) => void submitClient(e)} className="space-y-4">
              ...
              <div className="flex gap-2 pt-2">
                ...
                <PrimaryButton type="button" variant="outline" className="py-2.5 text-sm" onClick={closeClientModal} disabled={saving}>
                  {t('page.invoices.cancel', lang)}
                </PrimaryButton>
              </div>
            </form>
...
const AdminAnalyticsView: React.FC<{ lang: UiLang }> = ({ lang }) => (
  <section className="space-y-6" aria-labelledby="analytics-page-title">
    <PageHeader id="analytics-page-title" title={t('page.analytics.title', lang)} description={t('page.analytics.desc', lang)} />
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
...
const AdminAuditLogView: React.FC<{ lang: UiLang }> = ({ lang }) => {
  ...
  return (
    <section className="space-y-6" aria-labelledby="audit-page-title">
      <PageHeader id="audit-page-title" title={t('page.audit.title', lang)} description={t('page.audit.desc', lang)} />
      {err && <ErrorAlert message={err} onDismiss={() => setErr('')} />}
...
const AdminUsersView: React.FC<{ user: User; lang: UiLang }> = ({ user, lang }) => {
  ...
  return (
    <section className="space-y-6" aria-labelledby="users-page-title">
      <PageHeader
        id="users-page-title"
        title="Users"
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[minmax(0,1.6fr)_minmax(360px,1fr)] gap-6">
          ...
          {loading ? (
            ...
            <>
              <table className="w-full text-left">
                ...
                <thead className="bg-gray-50 dark:bg-gray-800/50 border-b border-gray-100 dark:border-gray-800">
                  <tr>
                    <th scope="col" className="px-6 py-3.5 text-xs font-semibold text-gray-500 uppercase tracking-wider">User</th>
                    <th scope="col" className="px-6 py-3.5 text-xs font-semibold text-gray-500 uppercase tracking-wider">Role</th>
                    <th scope="col" className="px-6 py-3.5 text-xs font-semibold text-gray-500 uppercase tracking-wider">Status</th>
                    <th scope="col" className="px-6 py-3.5 text-xs font-semibold text-gray-500 uppercase tracking-wider">Company</th>
                    <th scope="col" className="px-6 py-3.5 text-xs font-semibold text-gray-500 uppercase tracking-wider">Last login</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                  {rows.map((row) => {
                    ...
                    return (
                      ...
                      >
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-3">
                            <div className="a11y-avatar w-10 h-10 rounded-full overflow-hidden bg-primary-100 dark:bg-primary-900/20 flex items-center justify-center text-primary-700 text-xs font-bold shrink-0">
                              {row.avatarUrl ? (
                                <img
                                  src={row.avatarUrl}
                                  alt={`Profile picture of ${row.name}`}
        ...
        <Card className="p-5">
          {detailLoading ? (
            ...
            <div className="space-y-5">
              <div className="flex items-start gap-4">
                <div className="a11y-avatar w-16 h-16 rounded-2xl overflow-hidden bg-primary-100 dark:bg-primary-900/20 flex items-center justify-center text-primary-700 font-bold text-lg shrink-0">
...
const LegacyAdminSettingsView: React.FC<{
  ...
  return (
    <section className="max-w-3xl space-y-6" aria-labelledby="settings-legacy-title">
      <PageHeader id="settings-legacy-title" title={t('settings.title', lang)} description={companyId ? t('page.settings.companyDesc', lang) : t('page.settings.profileDesc', lang)} />
      {error && <ErrorAlert message={error} solution={errorSolution} onDismiss={() => { setError(''); setErrorSolution(undefined); }} />}
...
const AdminSettingsView: React.FC<{
  ...
  return (
    <section className="space-y-6" aria-labelledby="settings-page-title">
      <PageHeader id="settings-page-title" title={t('settings.title', lang)} description={t('settings.pageLead', lang)} />
      {error && (
        <ErrorAlert
          message={error}
          solution={errorSolution}
        ...
        <div className="space-y-6">
          <Card className="p-6">
            <div className="flex flex-col md:flex-row md:items-start gap-5">
              <div className="a11y-avatar w-24 h-24 rounded-3xl overflow-hidden bg-primary-100 dark:bg-primary-900/20 flex items-center justify-center shrink-0">
                <img src={profileAvatar} alt={`Profile picture of ${user.name}`} className="w-full h-full object-cover" />
              </div>
              <div className="flex-1 space-y-4">
                ...
                <div className="flex flex-wrap gap-2">
                  <input
                    ref={fileInputRef}
                    type="file"
                    aria-label={t('settings.uploadAvatar', lang)}
...
const ClientDashboardView: React.FC<{ user: User; lang: UiLang }> = ({ user, lang }) => {
  ...
  return (
    <section className="space-y-6" aria-labelledby="client-dashboard-title">
      <PageHeader id="client-dashboard-title" title={t('page.clientPortal.title', lang)} description={`${t('page.clientPortal.welcome', lang)}, ${user.name}`}
...
const ClientInvoicesView: React.FC<{ user: User; lang: UiLang }> = ({ user, lang }) => {
  ...
  return (
    <section className="space-y-6" aria-labelledby="client-invoices-title">
      <PageHeader id="client-invoices-title" title={t('page.clientInvoices.title', lang)} description={t('page.clientInvoices.desc', lang)} />
      <Card className="overflow-hidden">
        <table className="w-full text-left">
          <caption className="sr-only">{t('page.clientInvoices.title', lang)}</caption>
          <thead className="bg-gray-50 dark:bg-gray-800/50 border-b border-gray-100 dark:border-gray-800">
            <tr>
              <th scope="col" className="px-6 py-3.5 text-xs font-semibold text-gray-500 uppercase tracking-wider">{t('page.clientInv.document', lang)}</th>
              <th scope="col" className="px-6 py-3.5 text-xs font-semibold text-gray-500 uppercase tracking-wider text-right">{t('page.clientInv.amount', lang)}</th>
              <th scope="col" className="px-6 py-3.5 text-xs font-semibold text-gray-500 uppercase tracking-wider text-right">{t('page.clientInv.action', lang)}</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
            {data.map(inv => (
...
const ClientProjectsView: React.FC<{ lang: UiLang }> = ({ lang }) => (
  <section className="space-y-6" aria-labelledby="projects-page-title">
    <PageHeader id="projects-page-title" title={t('page.projects.title', lang)} description={t('page.projects.desc', lang)} />
...
const ClientSupportView: React.FC<{ lang: UiLang }> = ({ lang }) => (
  <section className="max-w-4xl mx-auto space-y-6" aria-labelledby="support-page-title">
    <div className="bg-gradient-to-br from-primary-600 to-primary-700 p-8 sm:p-10 rounded-2xl text-white shadow-xl shadow-primary-600/15 flex flex-col md:flex-row items-center gap-8">
      ...
      <div>
        <h1 id="support-page-title" className="text-2xl font-bold mb-2">{t('page.support.title', lang)}</h1>
      ...
      <Card className="p-6">
        ...
        <div className="space-y-2">
          {['Guide de facturation', 'Conditions SLA', 'Spécifications API'].map(title => (
            <button type="button" key={title} className="w-full flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-800 rounded-xl hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors group text-left">
              <span className="font-medium text-sm text-gray-700 dark:text-gray-300 group-hover:text-primary-600">{title}</span>
              <ExternalLink aria-hidden="true" size={14} className="text-gray-400 group-hover:text-primary-600" />
            </button>
          ))}
        </div>
      </Card>
