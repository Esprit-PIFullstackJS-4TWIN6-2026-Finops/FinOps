...
- **npm** 9+  
- **MySQL 8** with an empty database created (see `DB_NAME` in `backend/.env`)
...
3. **Configure environment**
   ...
   - **Backend**: copy `backend/.env.example` to `backend/.env` and set **MySQL** connection (`DB_HOST`, `DB_USER`, `DB_PASSWORD`, `DB_NAME`). Create the database on the server before starting the API.
...
>>>>>>> 910ac63 (Ajout de la partie deponses et factures)
